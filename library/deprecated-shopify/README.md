...
<?php
	...
	if (isset($_GET['code'])) { // if the code param has been sent to this page... we are in Step 2
		...
		$shopifyClient = new BasicShopifyAPI($_GET['shop'], "", SHOPIFY_API_KEY, SHOPIFY_SECRET);
		session_unset();
	...
	// if they posted the form with the shop name
		...
		$shopifyClient = new BasicShopifyAPI($shop, "", SHOPIFY_API_KEY, SHOPIFY_SECRET);
...
<?php
	...
	$sc = new BasicShopifyAPI($_SESSION['shop'], $_SESSION['token'], $api_key, $secret);
...
<?php
    ...
    $sc = new BasicShopifyAPI($_GET['shop'], '', SHOPIFY_API_KEY, SHOPIFY_SECRET);
